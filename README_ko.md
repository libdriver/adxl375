...
ADXL375는 작고 얇은 3축 가속도계로, 낮은 전력 소모와 최대 ±200g의 고해상도 측정을 제공합니다. 디지털 출력 데이터는 16비트, 2의 보수 데이터로 포맷되어 있으며 SPI(3 또는 4와이어) 또는 I2C 디지털 인터페이스를 통해 액세스할 수 있습니다. 32단계 선입선출(FIFO) 버퍼가 있는 통합 메모리 관리 시스템을 사용하여 데이터를 저장하여 호스트 프로세서 활동을 최소화하고 전체 시스템 전력 소모를 낮출 수 있습니다. 저전력 모드는 임계값 감지 및 매우 낮은 전력 소모에서 활성 가속도 측정을 통해 지능형 모션 기반 전력 관리를 가능하게 합니다.

LibDriver ADXL375는 LibDriver가 출시한 ADXL375의 전체 기능 드라이버입니다. 가속도 판독, 가속도 FIFO 모드 획득, 활동/비활동 상태 감지, 충격 감지 및 기타 기능을 제공합니다. LibDriver는 MISRA를 준수합니다.
...
/src 디렉토리에는 LibDriver ADXL375의 소스 파일이 포함되어 있습니다.

/interface 디렉토리에는 LibDriver ADXL375용 플랫폼 독립적인 IIC, SPI버스 템플릿이 포함되어 있습니다.

/test 디렉토리에는 LibDriver ADXL375드라이버 테스트 프로그램이 포함되어 있어 칩의 필요한 기능을 간단히 테스트할 수 있습니다.

/example 디렉토리에는 LibDriver ADXL375프로그래밍 예제가 포함되어 있습니다.

/doc 디렉토리에는 LibDriver ADXL375오프라인 문서가 포함되어 있습니다.

/datasheet 디렉토리에는 ADXL375데이터시트가 있습니다.
...
온라인 문서: [https://www.libdriver.com/docs/adxl375/index.html](https://www.libdriver.com/docs/adxl375/index.html).
